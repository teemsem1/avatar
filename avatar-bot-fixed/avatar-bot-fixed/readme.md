...
3. Configure the bot using environment variables (recommended, especially for deployment platforms like Railway):

   - Locally: copy `.env.example` to `.env` and fill in your values, or set them directly in your shell.
   - On Railway: go to your service > **Variables** tab and add:
     - `GUILD_ID` = your server ID
     - `BOT_TOKEN` = your Discord bot token

   `config.js` reads these automatically:
   ...
   ⚠️ Never commit a real bot token directly into `config.js` or any file pushed to a public GitHub repo — treat it like a password.
